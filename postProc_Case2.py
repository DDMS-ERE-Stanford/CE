...
def MPS():
    ...
    #    filename2 = output_dir + "m_c_" + str( ind + 1 ) + ".txt"
    ...
    plotUncertainty(15, 15, d_new_final, d_obs, channel_params, simul_params, 'uncertainty1', output_dir)
    plotUncertainty(15, 60, d_new_final, d_obs, channel_params, simul_params, 'uncertainty2', output_dir)
    plotUncertainty(60, 15, d_new_final, d_obs, channel_params, simul_params, 'uncertainty3', output_dir)
    plotUncertainty(60, 60, d_new_final, d_obs, channel_params, simul_params, 'uncertainty4', output_dir)

    plotUncertainty(15, 15, d_ini_final, d_obs, channel_params, simul_params, 'uncertainty1_ini', output_dir)
    plotUncertainty(15, 60, d_ini_final, d_obs, channel_params, simul_params, 'uncertainty2_ini', output_dir)
    plotUncertainty(60, 15, d_ini_final, d_obs, channel_params, simul_params, 'uncertainty3_ini', output_dir)
  ...
  #  m_c_avg = np.mean( m_c_final, axis = 1, keepdims = True )
   ...
   # m_thres, N_mis = threshold(m_true, m_c_avg, channel_params) 
   # f.write("Misclassified cells: %d \n" %N_mis)
    f.close()
 ...
 #   plotField( m_thres, simul_params, "update_threshold", output_dir )
    ...
    perm_new_avg = m_new_avg *  (channel_params["hperm"] - channel_params["lperm"]) + channel_params["lperm"]
 ...
 #   perm_c_avg = m_c_avg *  (channel_params["hperm"] - channel_params["lperm"]) + channel_params["lperm"]
  ...
  #  plotField( perm_c_avg, simul_params, "compare_field_avg", output_dir )
    plotField( perm_new_avg, simul_params, "updated_field_avg", output_dir )
