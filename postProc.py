...
def MPS():
    ...
    for ind in range (n_samples):
        filename = output_dir + "m_new_" + str( ind+70 ) + ".txt"
        m_new_final[:,ind] = np.loadtxt( filename )
        m_ini_final[:,ind:ind+1] = getMPSInitial( ind+70, channel_params, simul_params, MPS_file, data_dir ) 
        d_new_final[:,:,ind] = forwardWellTrans( m_new_final[:,ind], simul_params )
